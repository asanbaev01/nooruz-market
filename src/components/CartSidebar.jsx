  ...
  return (
    <>
      <style>{`
        ...
        .cart-overlay { animation: overlayFadeIn .35s ease-out both; }
        @keyframes sidebarIn { from { transform: translateX(100%); } to { transform: translateX(0); } }
        .cart-sidebar { animation: sidebarIn .5s cubic-bezier(.34,1.56,.64,1) both; will-change: transform; }
        @keyframes bagPulse {
          0%, 100% { transform: scale(1) rotate(0); }
          50% { transform: scale(1.12) rotate(-8deg); }
        }
        .cart-bag-icon { animation: bagPulse 2.5s ease-in-out infinite; }
        .cart-close { transition: all .4s cubic-bezier(.34,1.56,.64,1); }
        .cart-close:hover { transform: rotate(90deg) scale(1.15); color: #ef4444; }
        ...
        .cart-empty-text { animation: emptyFade .6s cubic-bezier(.34,1.56,.64,1) .2s both; }
        @keyframes itemIn {
        ...
        .cart-item.removing { animation: itemOut .35s ease-in forwards; pointer-events: none; }
        .cart-item:hover { transform: translateX(-4px); box-shadow: 0 8px 20px -8px rgba(124,108,255,.25); }
        .cart-item-img { transition: transform .5s cubic-bezier(.34,1.56,.64,1); }
        .cart-item:hover .cart-item-img { transform: scale(1.08) rotate(-3deg); }
        .qty-btn { transition: all .35s cubic-bezier(.34,1.56,.64,1); }
        .qty-btn:hover { transform: scale(1.15); background: #7C6CFF; color: white; border-color: #7C6CFF; }
        .qty-btn:active { transform: scale(.9); }
        .qty-btn.bounce { animation: qtyBounce .4s cubic-bezier(.34,1.56,.64,1); }
        @keyframes qtyBounce {
          0%, 100% { transform: scale(1); }
          40% { transform: scale(1.3); }
          70% { transform: scale(.9); }
        }
        .trash-btn { transition: all .35s cubic-bezier(.34,1.56,.64,1); }
        .trash-btn:hover { transform: scale(1.15) rotate(-8deg); background: #ef4444; color: white; border-color: #ef4444; }
        .trash-btn:active { transform: scale(.9); }
        @keyframes totalPulse {
          0%, 100% { transform: scale(1); }
          50% { transform: scale(1.03); }
        }
        .cart-total-value { animation: totalPulse 2.5s ease-in-out infinite; transition: color .3s ease; }
        .checkout-btn {
        ...
        .checkout-btn:hover { transform: translateY(-3px) scale(1.02); box-shadow: 0 15px 30px -10px rgba(124,108,255,.5); }
        .checkout-btn:active { transform: scale(.97); }
        .checkout-btn svg { transition: transform .4s cubic-bezier(.34,1.56,.64,1); }
        .checkout-btn:hover svg { transform: translateX(4px) rotate(-8deg); }
        .clear-btn { transition: all .3s ease; }
        .clear-btn:hover { transform: translateY(-2px); color: #ef4444; }
        .clear-btn svg { transition: transform .4s cubic-bezier(.34,1.56,.64,1); }
        .clear-btn:hover svg { transform: rotate(-15deg) scale(1.15); }
        @keyframes badgePop {
        ...
        .cart-items-scroll::-webkit-scrollbar-thumb:hover { background: rgba(124,108,255,.6); }
        @keyframes blobMove {
        ...
        <div className="cart-items-scroll flex-grow overflow-y-auto px-8 py-6 flex flex-col gap-4 relative z-10">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
              ...
              <p className="text-lg font-medium cart-empty-text">{t('cart.empty')}</p>
            ...
            cart.map((item, i) => {
              ...
              return (
                <div
                  key={key}
                  className={`cart-item flex items-center gap-4 p-4 bg-gray-50 rounded-xl ${isRemoving ? 'removing' : ''}`}
                ...
                >
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <p className="text-primary font-bold text-sm">
                      {item.price} {t('common.som')} × {item.quantity}
                    </p>
                    {/* ✅ КОТОРУЛГАН */}
                    <span className={`inline-block mt-1 text-xs font-semibold ${
                      item.isWholesale ? 'text-yellow-600' : 'text-primary'
                    }`}>
                      {item.isWholesale ? t('cart.wholesale') : t('cart.retail')}
                    </span>
                  </div>

                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button
                      onClick={() => handleQuantity(item.id || item._id, item.isWholesale, -1)}
        ...
        {cart.length > 0 && (
          <div className="relative z-10 border-t border-gray-100 px-8 py-6 bg-white">
            <div className="flex justify-between items-center mb-5">
              <span className="text-lg font-semibold text-gray-600">{t('cart.total')}:</span>
              <span className="cart-total-value text-2xl font-bold text-primary">
                {cartTotal.toLocaleString()} {t('common.som')}
              </span>
            </div>
